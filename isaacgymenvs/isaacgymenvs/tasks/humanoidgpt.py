...
class HumanoidGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.velocity, self.actions, self.dof_force_tensor)
...
def compute_reward(
    ...
    # Reward for speed: We want the humanoid to run as fast as possible
    # We consider only the horizontal velocity component (xy-plane)
    speed_reward = torch.norm(velocity[:, :2], p=2, dim=-1)
    
    # Reward transformation to emphasize the speed aspect
    speed_temp = 1.0
    transformed_speed_reward = torch.exp(speed_temp * speed_reward)
    
    # Penalty for excessive joint forces to encourage smooth motion
    force_penalty = torch.sum(torch.abs(dof_force_tensor), dim=-1)
    force_penalty_temp = 0.01
    transformed_force_penalty = torch.exp(-force_penalty_temp * force_penalty)
    
    # Penalty for high energy usage (i.e., large actions)
    action_penalty = torch.sum(torch.square(actions), dim=-1)
    action_penalty_temp = 0.01
    transformed_action_penalty = torch.exp(-action_penalty_temp * action_penalty)

    # Total reward combines all components
    total_reward = transformed_speed_reward + transformed_force_penalty + transformed_action_penalty
    
    # Aggregate the reward components in a dictionary for analysis
    reward_components = {
        "transformed_speed_reward": transformed_speed_reward,
        "transformed_force_penalty": transformed_force_penalty,
