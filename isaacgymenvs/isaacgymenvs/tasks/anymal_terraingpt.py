...
class AnymalTerrainGPT(VecTask):
...
def compute_reward(
    ...
    # Define temperatures for normalization
    lin_vel_temp = 0.5
    ang_vel_temp = 0.5
    
    # Compute velocity errors
    lin_vel_error = torch.norm(base_lin_vel[:, :2] - commands[:, :2], dim=-1)  # For x, y components
    ang_vel_error = torch.abs(base_ang_vel[:, 2] - commands[:, 2])  # For yaw component

    # Compute rewards for linear and angular velocity tracking
    lin_vel_reward = torch.exp(-lin_vel_temp * lin_vel_error)
    ang_vel_reward = torch.exp(-ang_vel_temp * ang_vel_error)

    # Total reward is the sum of both rewards
    total_reward = lin_vel_reward + ang_vel_reward

    # Return total reward and dictionary of components
    return total_reward, {
