...
def main(cfg):
    ...
    if cfg.human_feedback:
        with open(f'{prompt_dir}/human_feedback.txt', "r") as f:
            task_description = f.read()
    else:
        with open(f'{prompt_dir}/human_feedback_initial.txt', "r") as f:
            task_description = f.read()
    suffix = cfg.suffix
    model = cfg.model
    ...
    for iter in range(cfg.iteration):
        ...
        while total_samples_generated < cfg.sample:
            for attempt in range(1000):
                try:
                    ...
                    total_samples_generated += chunk_size
                    break
        ...
        for batch_start in range(0, total_samples, batch_size):
            ...
            for sample_i in range(batch_start, batch_end):
                ...
                # Extract python code from GPT response using regex patterns
                patterns = [
                    r'```python(.*?)```',
                    r'```(.*?)```',
                    r'"""(.*?)"""',
                ...
                # Remove unnecessary imports by keeping only the code starting at the first function definition
                ...
                # Save the new environment code when the output contains a valid code string
                ...
                # Select the freest GPU if not running on multiple GPUs
                if not cfg.run_on_multiple_GPUs:
                    set_freest_gpu()
        ...
        if not exec_success and cfg.sample != 1:
            ...
            logging.info("All code generation failed! Repeat this iteration from the current message checkpoint! Decrementing iteration number...")
            iter -= 1
            continue
