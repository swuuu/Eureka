...
class AnymalDClimbUpGPT(VecTask):
    ...
    def compute_reward(self):
        self.rew_buf[:], self.rew_dict = compute_reward(self.base_lin_vel, self.commanded_lin_vel, self.root_states, self.heights, self.device)
        self.extras['gpt_reward'] = self.rew_buf.mean()
        for rew_state in self.rew_dict: self.extras[rew_state] = self.rew_dict[rew_state].mean()
        self.gt_rew_buf, self.rew_dict = compute_success(self.base_lin_vel, 
                                                               self.base_ang_vel, 
                                                               self.commands, 
                                                               self.projected_gravity, 
                                                               self.root_states, 
...
def compute_reward(
    ...
    vel_diff = base_lin_vel - commanded_lin_vel
    velocity_tracking_reward = -torch.norm(vel_diff, p=2, dim=-1)
    
    # Normalize and transform velocity tracking reward
    temperature_velocity = 0.1  # temperature parameter for velocity transformation
    velocity_tracking_reward_exp = torch.exp(temperature_velocity * velocity_tracking_reward)

    # Compute height climbing reward
    step_height = torch.clamp(root_states[:, 2] - 0.5, min=-1, max=1)
    height_climbing_reward = step_height - heights

    # Normalize and transform height climbing reward
    temperature_height = 0.1  # temperature parameter for height transformation
    height_climbing_reward_exp = torch.exp(temperature_height * height_climbing_reward)
    
    # Combining both rewards
    total_reward = velocity_tracking_reward_exp + height_climbing_reward_exp

    # Return the total reward and a dictionary of individual rewards
    reward_components = {
        "velocity_tracking_reward": velocity_tracking_reward_exp,
        "height_climbing_reward": height_climbing_reward_exp
    }
